...
def create_compilation_sets(variants):
    ...
    result = sorted(sets.values(), key=cmp_to_key(sort_variants))
    # cmp=version_compare
    # sort variants in sets
    for r in result:
        r.variants = sorted(r.variants, key=cmp_to_key(sort_variants))
...
def compile(args):
    ...
    for s in sets:
        ...
        for i, variant in enumerate(s.variants):
            ...
            description = get_build_description(variant, registries, emsdk_cs=args.emsdk_cs)

            if not build_image(description, no_cache=args.no_cache):
                build_descriptions.append(description)
                log("{} - SUCCESS".format(v.name), True)
            else:
                log("{} - FAIL".format(v.name), True)
                set_compiled = False
                break
